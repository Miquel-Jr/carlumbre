...
<head>
  ...
  <style>
  .car-photos-carousel .carousel-item img {
    width: 100%;
    max-height: 420px;
    object-fit: cover;
...
<body>
  ...
  <div class="container mt-5">
    ...
    <h1 class="mb-4"><?= htmlspecialchars($car['brand'] ?? 'Auto') ?> con placa
      <?= htmlspecialchars($car['plate'] ?? '') ?> color
      <?= htmlspecialchars($car['color'] ?? '') ?> y año <?= htmlspecialchars($car['year'] ?? '') ?></h1>

    <!-- fotos del auto en carrusel -->
    <?php if (!empty($photos)): ?>
    <div id="carPhotosCarousel" class="carousel slide car-photos-carousel mb-4" data-bs-ride="carousel">
      <div class="carousel-indicators">
        <?php foreach ($photos as $index => $photo): ?>
        <button type="button" data-bs-target="#carPhotosCarousel" data-bs-slide-to="<?= (int) $index ?>"
          class="<?= $index === 0 ? 'active' : '' ?>" aria-current="<?= $index === 0 ? 'true' : 'false' ?>"
          aria-label="Foto <?= (int) ($index + 1) ?>"></button>
        <?php endforeach; ?>
      </div>
      ...
      <div class="carousel-inner rounded shadow-sm bg-light">
        ...
        <div class="carousel-item <?= $index === 0 ? 'active' : '' ?>">
          <img src="<?= htmlspecialchars($photo['photo_path']) ?>" class="d-block w-100"
            alt="Foto <?= (int) ($index + 1) ?> del auto <?= htmlspecialchars($car['plate'] ?? '') ?>">
        </div>
        <?php endforeach; ?>
      </div>
    ...
    <?php else: ?>
    <div class="alert alert-secondary mb-4">Este auto no tiene fotos registradas.</div>
    <?php endif; ?>


    <a href="/clients/cars/plates" class="btn btn-success mb-3">Volver</a>
