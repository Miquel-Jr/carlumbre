            ...
            <form method="GET" action="/products" class="d-flex gap-2">
                <select name="category" class="form-select" style="min-width: 180px;">
                    ...
                    <?php foreach (($categories ?? []) as $categoryOption): ?>
                        <option value="<?= htmlspecialchars($categoryOption) ?>" <?= ($selectedCategory ?? '') === $categoryOption ? 'selected' : '' ?>>
                        ...
                        <div class="card h-100 shadow-sm">
                            <div class="card-body d-flex flex-column gap-3">
                                ...
                                <div>
                                    ...
                                    <img
                                        src="<?= htmlspecialchars($part['image_2d']) ?>"
                                        alt="Imagen 2D de <?= htmlspecialchars($part['name']) ?>"
                                        class="part-image-2d preview-trigger"
                                        data-preview-type="2d"
                                        data-preview-src="<?= htmlspecialchars($part['image_2d']) ?>"
                                ...
                                <div>
                                    ...
                                    <?php if (!empty($part['model_3d'])): ?>
                                        <model-viewer
                                            src="<?= htmlspecialchars($part['model_3d']) ?>"
                                            alt="Modelo 3D de <?= htmlspecialchars($part['name']) ?>"
                                            class="part-view-3d preview-trigger"
                                            data-preview-type="3d"
                                            data-preview-src="<?= htmlspecialchars($part['model_3d']) ?>"
                                            data-preview-title="<?= htmlspecialchars($part['name']) ?> - Vista 3D"
                                            camera-controls
                                            auto-rotate
                                            shadow-intensity="1">
                                        </model-viewer>
